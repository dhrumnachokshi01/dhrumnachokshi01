...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Dhrumna\\Ahmedabad University\\Fourth year\\PSY\\INTENTIONAL BINDING TASK\\Intentional binding task_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "visual_stimuli" ---
    ...
    text = visual.TextStim(win=win, name='text',
        text='Report the delay between the key press and the visual stimuli (0-999). Click submit after entering the value\n',
        font='Open Sans',
    ...
    for thisTrial in trials:
        ...
        # --- Prepare to start Routine "visual_stimuli" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('visual_stimuli.started', globalClock.getTime())
        polygon_3.setPos((0, 0))
        polygon_3.setSize((0.5, 0.5))
        polygon_3.setOri(0.0)
        # keep track of which components have finished
        visual_stimuliComponents = [polygon_3]
        for thisComponent in visual_stimuliComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        while continueRoutine:
            ...
            if polygon_3.status == NOT_STARTED and tThisFlip >= delay-frameTolerance:
                # keep track of start time/frame for later
        ...
        # --- Ending Routine "visual_stimuli" ---
        for thisComponent in visual_stimuliComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('visual_stimuli.stopped', globalClock.getTime())
        # the Routine "visual_stimuli" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
